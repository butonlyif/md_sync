...
class MDSyncApp:
    ...
    def add_project(self):
        ...
        ttk.Entry(path_frame, textvariable=path_var, width=50).pack(side=tk.LEFT)
        ttk.Button(path_frame, text="浏览...", command=lambda: self.select_project_path(path_var)).pack(side=tk.LEFT, padx=5)

        ttk.Label(dialog, text="Obsidian 子目录:").pack(pady=5)
        subdir_var = tk.StringVar()
        subdir_entry = ttk.Entry(dialog, textvariable=subdir_var, width=50)
        subdir_entry.pack(pady=5)
        ttk.Label(dialog, text="（留空则复制到 Obsidian 根目录）").pack(pady=(0, 5))
